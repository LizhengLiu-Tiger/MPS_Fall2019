...
// Lab 4 task 1, ADC sampling
//Author: Tiger and Frank
//--------------------------------
//
//
...
void configureADC();
float count = 0;//count the number of samples
float high = 0;//highest sample value
float low = 5;//lowest sample value
float total;//sum of the historical readings
float ave=0;//average of the readings
int flag =0;//for the push button
...
int main(void)
{
	...
	while(1)
	{
		if(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_7))//push button enable the reading
		{
			flag =1;
			HAL_Delay(400);
		}
